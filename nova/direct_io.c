...
static ssize_t do_nova_cow_file_write_async(struct file *filp,
    struct page **kaddr, size_t len, loff_t ppos)
{
...
out:
  ...
  if (try_inplace) 
    return do_nova_inplace_file_write_async(filp, kaddr, len, ppos);
                                  ...
                                  size_t len, loff_t ppos)
{
    ...
    if (test_opt(inode->i_sb, DATA_COW)) {
        ...
        ret = do_nova_inplace_file_write_async(filp, kaddr, len, ppos);
        inode_unlock(inode);
    }
...
out:
    ...
    struct nova_inode_info *ino_info = container_of(inode, struct nova_inode_info, vfs_inode);;
...
ssize_t nova_direct_IO(struct kiocb *iocb, struct iov_iter *iter)
{
    ...
    iv = iter->iov;
    nova_info("%s: iocb->pos: %ld, iter->iov_offset: %ld, end : %lu\n",__func__,iocb->ki_pos,iter->iov_offset,end);
    ...
    if (!is_sync_kiocb(iocb))
    {
        ...
        nova_info("%s :inode.i_size : %lu,sb.s_blocksize_bits : %d , size %lu\n",__func__, inode->i_size, sb->s_blocksize, size);
        ...
        */
        ...
        while (seg < nr_segs)
        {
            ...
            nova_info("%s  :nr_segs:%lu, segs %d iov_base %p, iov_len %lu,ki_pos %llu\n",__func__, 
                        nr_segs,seg, io_work->my_iov.iov_base, io_work->my_iov.iov_len, io_work->ki_pos);
            

            first_blk = pos >> PAGE_SHIFT;
            off = pos & (PAGE_SIZE - 1);
            nr = DIV_ROUND_UP( iv->iov_len+ off, PAGE_SIZE);
            io_work->first_blk = first_blk;
            io_work->blknr = nr;
            nova_info("%s  : first_blk :%lu , off :%lu, pos %lu, nr %lu\n",__func__, first_blk, off, pos, nr);
            ...
            seg++;
             ...
             if(need_grow){
                ...
                if(seg == nr_segs){
                    ...
                    io_work->first_blk = first_blk;
                    io_work->blknr = nr;
                    nova_info("%s : first_blk : %lu , nr : %lu , seg : %lu",__func__,first_blk,nr,seg);
                }
                else
                    continue;
            ...
            nova_info("%s wtbit:%lu, wkbit:%lu\n",__func__,wtbit,wkbit);
            ...
            if (wtbit < first_blk + nr || wkbit < first_blk + nr)
            {
                ...
                { // 01 ,other I/O working on,insert I/O to waitqueue
                    wtbit = first_blk;
                    ...
                    while (async_pos != &ino_info->aio.i_waitque)
                    {
                        ...
                        if (f_blk + contn->blknr >= first_blk && f_blk <= first_blk + nr)
                        {
                            if(async_pos->next != &ino_info->aio.i_waitque){
                                ...
                                {
                                    async_pos = async_pos->next;
                                    /*delet contn from waitqueue*/
                                    list_del(&contn->aio_waitq);
                                    /*merge contn to tmp_contn*/
                                    __list_splice(&contn->aio_conflicq, tmp_contn->aio_conflicq.prev, &tmp_contn->aio_conflicq);
                                    /*head I/O node of conflict queue save the min first_blk and max blknr*/
